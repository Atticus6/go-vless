...
启动后向 dashboard 注册并常驻心跳：成功后每 30 分钟全量同步一次 UUID（服务端下发周期为准），失败每 60 秒重试；三项缺任一则只启动代理，不注册。
...
能看到：隧道开没开、隧道地址、能用的访问地址、服务器对外显示的 IP、**每个用户用了多少流量**、内存占用、运行了多久，以及 **dashboard 注册连接状态**（`register` 段：是否启用、上次同步成功时间、最近错误、下次同步倒计时、已同步用户数）。

流量说明：`up` = 用户发出去的，`down` = 用户收到的（另有 `upBytes` / `downBytes` 原始字节数，供 dashboard 落库）；**重启后清零**（Serverless 睡眠唤醒后也一样）。
